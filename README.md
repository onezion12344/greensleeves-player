...
Every track in this collection is generated from the *same* machine-readable score — the same melody events, the same bar grid, the same tempo — and only the orchestration changes. Because they share one rhythmic skeleton, any two versions can be swapped for each other without the timing moving. That is the point of the experiment.
...
[v1.0.0 release](https://github.com/onezion12344/greensleeves-player/releases/tag/v1.0.0) — there is no bundled archive, and nothing is zipped.
...
| **WAV** (44.1 kHz / 16-bit stereo) | The masters, loudness-normalised | [v1.0.0 release](https://github.com/onezion12344/greensleeves-player/releases/tag/v1.0.0) — one asset per track | As release assets they stay individually downloadable without putting the whole suite into git history forever |
...
- The pipeline that generated these arrangements is **not published** and is not open
...
这个合辑里的每一轨都来自**同一份机器可读的乐谱** —— 同样的旋律事件、同样的小节网格、同样的速度，变的只有配器。因为九版共享同一个节奏骨架，任意两版都可以互换而不产生错位。这正是这次实验想验证的事。
...
- **格式**：`tracks/` 是 MP3（192 kbps，可直接在浏览器播放）；WAV 母带
  （44.1 kHz / 16-bit stereo）在 [v1.0.0 release](https://github.com/onezion12344/greensleeves-player/releases/tag/v1.0.0) 里，**每轨单独一个附件**，没有打包压缩包；
  `midi/` 是每版的分谱 MIDI。
